...
export async function getGoogleAccessTokenForAccount(accountId) {
  const result = await pool.query(
    `
      SELECT
        id AS connected_account_id,
  ...
  if (account.status !== "connected") {
    throw new Error(
      `Google account ${accountId} is not connected`,
    );
  }
  ...
  if (!token) {
    throw new Error(
      `Google account ${accountId} did not provide an access token`,
    );
  }
